...
import type { CompareGroupDraft, CompareResponse } from '@/types'
...
function nowRange(seconds: number): [Date, Date] {
  const end = Date.now()
  return [new Date(end - seconds * 1000), new Date(end)]
}

/** 把持久化的时间戳还原为 Date */
function hydrateDraft(d: any): CompareGroupDraft {
  return {
    name: String(d?.name ?? ''),
...
function defaultBatchGroups(): CompareGroupDraft[] {
  return [
    { name: '基准组', range: nowRange(120), device_ids: [], device_types: [] },
    { name: '对比组', range: [new Date(Date.now() - 240_000), new Date(Date.now() - 120_000)], device_ids: [], device_types: [] },
  ]
}
...
interface PersistedState {
  mode: 'single' | 'batch'
  view: 'setup' | 'result'
  single: CompareGroupDraft
  batch: CompareGroupDraft[]
...
export const useCompareStore = defineStore('compare', () => {
  ...
  const mode = ref<'single' | 'batch'>(saved.mode === 'batch' ? 'batch' : 'single')
  const view = ref<'setup' | 'result'>(saved.view === 'result' ? 'result' : 'setup')
  const single = ref<CompareGroupDraft>(
    saved.single ? hydrateDraft(saved.single) : { name: '单次查看', range: nowRange(60), device_ids: [], device_types: [] },
  )
  const batch = ref<CompareGroupDraft[]>(
    Array.isArray(saved.batch) && saved.batch.length
      ? saved.batch.map(hydrateDraft)
      : defaultBatchGroups(),
  )
  const baselineIndex = ref<number>(saved.baselineIndex ?? 0)
  const metric = ref<string>(saved.metric ?? 'production')
  /** 按指标查看时勾选参与对比的组（按组 index） */
  const selected = ref<number[]>(Array.isArray(saved.selected) ? saved.selected : [])
  ...
  function addGroup() {
    if (batch.value.length >= 10) return
    batch.value.push({
  ...
  function removeGroup(index: number) {
    if (batch.value.length <= 1) return
  ...
  async function runCompare(groups: CompareGroupDraft[], baseline: number) {
    loading.value = true
    error.value = null
    try {
      const payload = {
      ...
      const { data } = await axios.post<CompareResponse>('/api/production/compare', payload)
      result.value = data
      view.value = 'result'
